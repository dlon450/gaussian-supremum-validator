"""Distributionally robust (Wasserstein) chance-constrained knapsack.

Solves a binary knapsack with a DRO chance constraint using Gurobi with lazy
constraints generated by :func:`callback`. ``DRO_solver`` returns, for each
ambiguity radius in ``para``, the selected items concatenated into one array.
