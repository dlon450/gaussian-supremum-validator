...
def algo1(delta, c, b, alpha, n, d, ksi, beta, K):
    l = int(n / K)
    delta = np.arange(np.ceil((n - l) * (1 - alpha)).astype(int), (n - l) + 1) 
    m = len(delta)
    ...
    for i in range(m):
        ...
        for j in range(K):
            ...
            x_ = CCP_SAA(delta[i], c, b, ksi_train, alpha, d, n - l)
    ...
    x = CCP_SAA(delta_star, c, b, ksi, alpha, d, n)
    return x 
...
def algo2(delta, c, b, alpha,n,d,ksi,beta,B):
    ...
    delta = np.arange(np.ceil((n) * (1 - alpha)).astype(int), (n) + 1) 
    for i in range(B):
        for j in range(n):
            if j in R[i]:
                setup[i,j] = 1
    ...
    for i in range(m):
        ...
        for j in range(B):
            ...
            x_ = CCP_SAA(delta[i], c, b, ksi_train, alpha, d, n)
            P = np.dot(ksi_validation, x_ ) - b
            cost += np.dot(c,x_)
            p = len(P[P < 0]) / denom[j]
            V[j,i] = p
...
def algo3(delta, c, b, alpha,n,d,ksi,beta,n1,n2,K):
    ...
    for i in range(m):
        x_ = CCP_SAA(delta[i], c, b, ksi_train, alpha, d, n1)
        xx[:,i] = x_
        for j in range(K):
...
def GS(delta, c, b, alpha, n, d, ksi, beta, n1, n2):
    ...
    ksi_train = ksi_shuffle[:n1]
    ksi_v = ksi_shuffle[n1:]
    xx = CCP_SAA(delta, c, b, ksi_train, alpha, d, n1)
    C = np.dot(c,xx)
