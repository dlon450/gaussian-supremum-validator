...
def algo1(delta, c, b, alpha,n,d,ksi,beta,K):
    ...
    for i in range(m):
        ...
        for j in range(K):
            ...
            x_ = CCP_SO(delta[i], c, b, ksi_train, alpha, d, n - l)
    ...
    x = CCP_SO(delta_star, c, b, ksi, alpha, d, n)
    return x 
...
def algo2(delta, c, b, alpha,n,d,ksi,beta,B):
    ...
    for i in range(m):
        ...
        for j in range(B):
            ...
            x_ = CCP_SO(delta[i], c, b, ksi_train, alpha, d, n)
            P = np.dot(ksi_validation, x_ ) - b
            cost += np.dot(c,x_)
            p = len(P[P < 0]) / denom[j]
            V[j,i] = p
...
def algo3(delta, c, b, alpha,n,d,ksi,beta,n1,n2,K):
    ...
    for i in range(m):
        x_ = CCP_SO(delta[i], c, b, ksi_train, alpha, d, n1)
        xx[:,i] = x_
        for j in range(K):
...
def GS(delta, c, b, alpha, n, d, ksi, beta, n1, n2):
    ...
    ksi_train = ksi_shuffle[:n1]
    ksi_v = ksi_shuffle[n1:]
    xx = CCP_SO(delta, c, b, ksi_train, alpha, d, n1)
    C = np.dot(c,xx)
...
for n in [100, 200, 300, 400, 500, 600, 700, 800, 900, 1000]:
    ...
    for K in [3, 5, 10]:
        ...
        result = perform_stats(c,N,d,n,alpha,beta,K,B,n1,n2,delta=delta_SO,b=b,dim=dim)
