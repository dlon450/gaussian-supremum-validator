...
def CCP_DRO_moment(para, c, b, data, alpha, d, n):
    ...
    for k in range(mesh_size):
        ...
        # Constraints
        constraints = [
            np.sqrt(alpha / (1 - alpha)) * mu_hat @ x_dro + cp.trace(Sigma_hat @ W) + rho[k] * cp.norm((A @ sqrt_cov).T @ vec_q) + tilde_c + eta / 4 <= 0,
...
def algo1(delta, c, b, alpha,n,d,ksi,beta,K):
    ...
    for i in range(m):
        ...
        for j in range(K):
            ...
            x_ = CCP_DRO_moment(delta[i], c, b, ksi_train, alpha, d, n - l)
    ...
    x = CCP_DRO_moment(delta_star, c, b, ksi, alpha, d, n)
    return x 
...
def algo2(delta, c, b, alpha,n,d,ksi,beta,B):
    ...
    for i in range(m):
        ...
        for j in range(B):
            ...
            x_ = CCP_DRO_moment(delta[i], c, b, ksi_train, alpha, d, n)
            P = np.dot(ksi_validation, x_ ) - b
            cost += np.dot(c,x_)
            p = len(P[P < 0]) / denom[j]
            V[j,i] = p
...
def algo3(delta, c, b, alpha,n,d,ksi,beta,n1,n2,K):
    ...
    for i in range(m):
        x_ = CCP_DRO_moment(delta[i], c, b, ksi_train, alpha, d, n1)
        xx[:,i] = x_
        for j in range(K):
...
def GS(delta, c, b, alpha, n, d, ksi, beta, n1, n2):
    ...
    ksi_train = ksi_shuffle[:n1]
    ksi_v = ksi_shuffle[n1:]
    xx = CCP_DRO_moment(delta, c, b, ksi_train, alpha, d, n1)
    C = np.dot(c,xx)
