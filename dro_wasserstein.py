"""Experiment: Wasserstein-ball DRO chance constraint.

Runs the Gaussian-supremum validator and baselines (dim == 4) for the
Wasserstein-DRO solver across a range of (small) data sizes.

Run with:  python dro_wasserstein.py
"""

import numpy as np
import pandas as pd

from gsv.formulations import CCP_DRO_wasserstein
...
for n in [20, 40, 60, 80, 100, 120, 140, 160, 180, 200]:
    ...
    #     B = K
        ...
        result = run_experiment(CCP_DRO_wasserstein,c,N,d,n,alpha,beta,K,B,n1,n2,delta=delta,b=b,dim=dim)
